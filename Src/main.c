...
//    HAL_UART_Receive_DMA(&huart3, receive_buf, 15);
	...
	/* add threads, ... */
	status = xTaskCreate(Task_Control, "Control Task", 250, NULL, 2,
			&task_control);
	configASSERT(status == pdPASS);
	status = xTaskCreate(Task_Keyboard, "Keyboard Task", 250, NULL, 2,
			&task_keyboard);
	configASSERT(status == pdPASS);
	status = xTaskCreate(Task_Climbing, "Climbing Task", 250, NULL, 2,
			&task_climbing);
	configASSERT(status == pdPASS);
	status = xTaskCreate(Task_Joystick, "Joystick Task", 250, NULL, 2,
			&task_joystick);
	configASSERT(status == pdPASS);
	status = xTaskCreate(Task_Climb_Sensor, "Climb Sensor Task", 300, NULL, 2,
			&task_climb_sensor);
	configASSERT(status == pdPASS);
	status = xTaskCreate(Task_NormalDrive, "Normal Drive Task", 250, NULL, 2,
			&task_normalDrive);
	configASSERT(status == pdPASS);
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
	...
	case AD_BUSY_Pin: {
		...
		xQueueSendFromISR(queue_joystick, (void* )&joystick_handler_irq,
	...
	if (hcan == &hcan1) {
		...
		if (incoming[1] == ENC_ADDR_LEFT) {
			...
			encoderBack.encoder_pos = (uint32_t) ((4096 * BACK_GEAR_RATIO)
					- encoderBack.encoder_pos) % (4096 * BACK_GEAR_RATIO);
	...
	if (huart->Instance == USART3) {
		...
		if ((uint8_t) sum == receive_buf[14]) {
			...
			if (receive_buf[0] == 0xAA && receive_buf[1] == 0xA4
					&& receive_buf[3] == 0x00 && receive_buf[4] == 0x00) {
	...
	if (huart->Instance == USART1) {
		...
		if (distance != 0) {
			...
			if (distanceNoNoise >= 20 && lifting_mode == NORMAL) {
				...
				lifting_mode = STOP;
				xTaskNotifyFromISR(task_normalDrive, 0, eNoAction,
						&xHigherPriorityTaskWoken);
